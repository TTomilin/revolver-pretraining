...
if __name__ == "__main__":
    ...
    parser.add_argument("--generalized_env", default="Ant-v2-leg-emerge")  # OpenAI gym environment name
    ...
    env = make_generalized_envs.generalized_envs[args.generalized_env](interp_param=args.interp,
    ...
    evaluations = [eval_policy(policy, args.generalized_env, args.interp, args.seed, log_string, render=False,
                               tmp_file_dir=tmp_file_dir)]
    if args.eval:
        exit()
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if done:
            ...
            log_string("Total T: {} Episode Num: {} Episode T: {} Reward: {:.3f}".format(t + 1, episode_num + 1,
                                                                                         episode_timesteps,
                                                                                         episode_reward))
            # Reset environment
        ...
        if (t + 1) % args.eval_freq == 0:
            evaluation = eval_policy(policy, args.generalized_env, args.interp, args.seed, log_string,
                                     tmp_file_dir=tmp_file_dir)
